...
public class LinkedList {
    ...
    void insertBeginning( int processId , int burstTime, int priority) {
        // Each node will represent a movie and contain Movie Title, Director, Year of Release, and Rating.
        TaskNode s = new TaskNode(processId,burstTime,priority);
        if (this.head == null) {
            s.next = s;
            this.head = s;
            this.tail = s;
    ...
    void insertEnd(int processId , int burstTime, int priority) {
        TaskNode s = new TaskNode( processId ,  burstTime,  priority);

        if (tail == null) {
            insertBeginning( processId,  burstTime, priority);
        } else {
           tail.next = s;
           s.next = head;
           tail = s;
    ...
    void insertSpecific(int processId , int burstTime, int priority, int position) {
...
//    }
    void printDetails() {
        ...
        do {
            ...
            System.out.println("Process burst time " + temp.burstTime);
            System.out.println("Process priority " + temp.priority + "\n");
            temp = temp.next;
        } while (temp != head);
    }
}
